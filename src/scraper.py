...
from src.adaptive_delay import AdaptiveDelayManager, SellerInfoCache


# 动态读取浏览器模式，确保使用.env文件中的最新配置
def get_headless_mode():
    """从.env文件动态读取浏览器模式，确保每次启动浏览器时都使用最新配置"""
    env_config = dotenv_values(".env")
    return env_config.get("RUN_HEADLESS", "true").lower() != "false"
...
async def scrape_xianyu(task_config: dict, debug_limit: int = 0):
    ...
    # === 初始化自适应延迟和缓存管理器 ===
    delay_manager = AdaptiveDelayManager(initial_min=15, initial_max=25)
    seller_cache = SellerInfoCache(cache_ttl_hours=24)

    print(f"\n{'='*70}")
    print("🚀 智能优化功能已启用:")
    print("   ✅ 自适应延迟管理 - 根据成功率动态调整速度")
    print("   ✅ 卖家信息缓存 - 避免重复采集，提升效率")
    print(f"{'='*70}\n")
    ...
    if USE_PROXY_POOL:
        ...
        print("│ 🔄 代理池模式已启用                                                  │")
        print(f"└{'─'*58}┘\n")
        proxy_pool = ProxyPool(proxy_file=PROXY_POOL_FILE)
        await proxy_pool.initialize()
        proxy_pool.print_status()

    async with async_playwright() as p:
        # 动态读取浏览器模式，确保使用.env文件中的最新配置
        headless_mode = get_headless_mode()
        print(f"   -> 浏览器模式: {'无头模式' if headless_mode else '有头模式'}")

        if LOGIN_IS_EDGE:
            browser = await p.chromium.launch(headless=headless_mode, channel="msedge")
        else:
            # Docker环境内，使用Playwright自带的chromium；本地环境，使用系统安装的Chrome
            if RUNNING_IN_DOCKER:
                browser = await p.chromium.launch(headless=headless_mode)
            else:
                browser = await p.chromium.launch(headless=headless_mode, channel="chrome")
        ...
        # 如果启用代理池，获取代理并应用
        if proxy_pool:
            current_proxy_info = proxy_pool.get_proxy(strategy=PROXY_STRATEGY)
            if current_proxy_info:
                # Playwright 代理配置需要分离认证信息
                proxy_config = {
                    'server': f"http://{current_proxy_info.host}:{current_proxy_info.port}"
                }
                # 如果有用户名和密码，添加到配置中
                if current_proxy_info.username and current_proxy_info.password:
                    proxy_config['username'] = current_proxy_info.username
                    proxy_config['password'] = current_proxy_info.password

                browser_context_config['proxy'] = proxy_config

                # 代理模式下忽略HTTPS错误（代理可能导致SSL证书问题）
                browser_context_config['ignore_https_errors'] = True

                # 醒目地显示代理信息
                print(f"\n┌{'─'*68}┐")
                print(f"│ 🌐 当前代理: {current_proxy_info.host}:{current_proxy_info.port}{' '*33}│")
                print(f"│    用户: {current_proxy_info.username}{' '*54}│")
                print(f"└{'─'*68}┘\n")

                current_proxy_url = current_proxy_info.url
            else:
                print(f"\n{'='*70}")
                print(f"⚠️  警告: 无可用代理，将直连访问（未使用代理）")
                print(f"{'='*70}\n")
                current_proxy_url = None
        else:
            print(f"\n{'='*70}")
            print(f"⚠️  代理池未启用，当前为直连模式")
            print(f"{'='*70}\n")
        ...
        try:
            ...
            await random_sleep(3, 6)  # 增加初始等待时间
            ...
            # 显示当前使用的代理IP（如果有）
            if proxy_pool and current_proxy_url:
                print(f"\n📍 当前代理: {current_proxy_url}\n")
            ...
            for page_num in range(1, max_pages + 1):
                ...
                for i, item_data in enumerate(basic_items, 1):
                    ...
                    # 访问详情页前的延迟
                    await delay_manager.wait("访问详情页前")
                    ...
                    try:
                        ...
                        if detail_response.ok:
                            ...
                            if user_id:
                                # 先尝试从缓存获取
                                cached_profile = seller_cache.get(str(user_id))
                                if cached_profile:
                                    user_profile_data = cached_profile
                                else:
                                    # 缓存未命中，采集新数据
                                    user_profile_data = await scrape_user_profile(context, str(user_id))
                                    seller_cache.set(str(user_id), user_profile_data)
                            else:
                            ...
                            # 显示当前使用的代理IP
                            if proxy_pool and current_proxy_url:
                                log_time(f"商品处理完毕。累计处理 {processed_item_count} 个新商品。（代理: {current_proxy_url}）")
                            else:
                                log_time(f"商品处理流程完毕。累计处理 {processed_item_count} 个新商品。")

                            # 使用自适应延迟
                            await simulate_human_behavior(page)
                            await delay_manager.wait("商品处理后")
                        else:
                            print(f"   错误: 获取商品详情API响应失败，状态码: {detail_response.status}")
                            delay_manager.record_failure()  # 记录失败
                            if AI_DEBUG_MODE:
                ...
                # 在处理完一页所有商品后，翻页前，增加一个更长的"休息"时间
                if not stop_scraping and page_num < max_pages:
                    print(f"--- 第 {page_num} 页处理完毕，准备翻页。执行一次页面间的长时休息... ---")
                    await simulate_human_behavior(page)
                    await delay_manager.wait("翻页前休息")
            ...
            print(f"\n{'='*70}")
            print("📊 智能优化统计报告:")
            delay_manager.print_stats()
            seller_cache.print_stats()
            print(f"{'='*70}\n")
            ...
            if proxy_pool:
                ...
                if current_proxy_url:
                    print(f"✅ 任务完成 - 本次使用代理: {current_proxy_url}")
                else:
                    print(f"⚠️  任务完成 - 未使用代理（直连模式）")
                print(f"{'='*70}\n")
                print("[代理池] 保存代理状态...")
