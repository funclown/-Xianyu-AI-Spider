...
class NotificationSettings(BaseModel):
    # 移除的通知渠道：Ntfy, Gotify, Bark, Telegram (2025-12-29)
    # 保留的通知渠道：企业微信、钉钉、飞书、通用Webhook

    WX_BOT_URL: Optional[str] = None
    DINGTALK_BOT_URL: Optional[str] = None
...
def load_notification_settings():
    ...
    return {
        ...
        "DINGTALK_BOT_URL": config.get("DINGTALK_BOT_URL", ""),
        "FEISHU_BOT_URL": config.get("FEISHU_BOT_URL", ""),
...
def save_notification_settings(settings: dict):
    """Save notification settings to .env file while preserving comments and format"""
    env_file = ".env"

    # Settings that this function handles
    setting_keys = set([
        "WX_BOT_URL", "DINGTALK_BOT_URL", "FEISHU_BOT_URL", "WEBHOOK_URL",
    ...
    # First pass: update existing lines while preserving comments and structure
    for line in env_lines:
        ...
        if '=' in line:
            ...
            # If this is a notification setting, update it
            if key in setting_keys:
                new_value = settings.get(key, "")
                # Format PCURL_TO_MOBILE as lowercase boolean
            ...
            # Keep lines without = as-is
            new_lines.append(line)

    # Second pass: add any new notification settings that weren't in the file
    for key in setting_keys:
        if key not in written_keys and key in settings:
...
def save_ai_settings(settings: dict):
    """Save AI model settings to .env file while preserving comments and format"""
    env_file = ".env"
    ...
    for line in env_lines:
        ...
        if '=' in line:
            ...
            # If this is an AI setting, update it
            if key in setting_keys:
                new_value = settings.get(key, "")
                new_lines.append(f"{key}={new_value}\n")
                written_keys.add(key)
    ...
    # Second pass: add any new AI settings that weren't in the file
    for key in setting_keys:
        if key not in written_keys and key in settings:
...
app.add_middleware(
    ...
    max_age=None  # Session永不过期，直到浏览器关闭
)

# 异常处理器：对于401错误，如果是浏览器请求则重定向到登录页
...
async def reload_scheduler_jobs():
    ...
    # ========================================================================
    # [已移除 - 2025-12-29] 代理池自动更新任务
    # 原功能：根据配置自动定时提取和更新代理池
...
# ========================================================================
# [已移除 - 2025-12-29] 代理池相关常量定义
# 原常量：
#   PROXY_POOL_CONFIG_FILE = "proxy_pool_config.json"
#   PROXY_POOL_ACTIVITY_FILE = "proxy_pool_activity.jsonl"
...
async def update_task_enabled_status(task_id: int, enabled: bool):
    """更新 config.json 中指定任务的 enabled 状态。"""
    try:
        async with aiofiles.open(CONFIG_FILE, 'r', encoding='utf-8') as f:
            tasks = json.loads(await f.read())
        ...
        if 0 <= task_id < len(tasks):
            ...
            async with aiofiles.open(CONFIG_FILE, 'w', encoding='utf-8') as f:
                await f.write(json.dumps(tasks, ensure_ascii=False, indent=2))
            print(f"任务 {task_id} 的启用状态已更新为: {enabled}")
    except Exception as e:
        print(f"更新任务 {task_id} 启用状态时出错: {e}")
...
async def stop_all_tasks(username: str = Depends(verify_credentials)):
    """停止所有正在运行的任务，并禁用所有定时任务。"""
    global scraper_processes
    stopped_count = 0
    failed_count = 0

    # 复制所有任务ID列表，避免在迭代过程中修改字典
    task_ids = list(scraper_processes.keys())
    ...
    for task_id in task_ids:
        try:
            ...
            await update_task_running_status(task_id, False)
            # 禁用任务
            await update_task_enabled_status(task_id, False)
    ...
    # 清空进程字典
    scraper_processes.clear()

    # 重新加载调度器（会移除所有被禁用的任务）
    await reload_scheduler_jobs()

    return {
        "message": f"已停止 {stopped_count} 个任务，禁用所有定时任务。",
...
@app.get("/api/logs/stats", response_model=dict)
async def get_logs_stats(username: str = Depends(verify_credentials)):
    """
    获取日志文件夹的统计信息。
    """
    logs_dir = "logs"
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"获取日志统计信息时出错: {e}")


@app.post("/api/logs/cleanup-old", response_model=dict)
async def cleanup_old_logs(days: int = 3, username: str = Depends(verify_credentials)):
    """
    清理超过指定天数的旧日志文件。
    默认保留最近3天的日志。
    """
    ...
    try:
        ...
        for filename in os.listdir(logs_dir):
            if filename.endswith('.log') and filename != 'scraper.log':
                file_path = os.path.join(logs_dir, filename)
                stat = os.stat(file_path)
        ...
        return {
            ...
            "message": f"已删除 {deleted_count} 个超过 {days} 天的旧日志文件，释放 {round(freed_space_mb, 2)} MB 空间。"
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"清理旧日志时出错: {e}")
...
async def get_system_status(username: str = Depends(verify_credentials)):
    ...
    if login_state["exists"]:
        try:
            with open("xianyu_state.json", "r", encoding="utf-8") as f:
                state_data = json.load(f)

            cookies = state_data.get("cookies", [])
            login_state["total_cookies"] = len(cookies)
            login_state["cookie_count"] = len(cookies)
            login_state["has_cookies"] = len(cookies) > 0

            # 检查关键cookie是否存在
            key_cookies = ["tracknick", "unb", "sgcookie", "cookie2"]
            found_key_cookies = []
            ...
            for cookie in cookies:
                ...
                # 获取用户名
                if name == "tracknick":
                    login_state["username"] = cookie.get("value", "")

                # 检查关键cookie
                if name in key_cookies:
                    found_key_cookies.append(name)

            login_state["expired_cookies"] = expired_count
            # 判断登录状态是否有效：有cookies，关键cookie都在，且没有过多过期cookie
            login_state["valid"] = (
                len(cookies) > 0 and
                len(found_key_cookies) >= 2 and
                expired_count < len(cookies) / 2  # 过期cookie少于一半
            )
        except Exception as e:
            print(f"读取闲鱼登录状态失败: {e}")
    ...
    status = {
        ...
        "env_file": {
            ...
            "dingtalk_bot_url_set": bool(env_config.get("DINGTALK_BOT_URL")),
            "feishu_bot_url_set": bool(env_config.get("FEISHU_BOT_URL")),
...
@app.get("/api/openai/credit")
async def get_openai_credit(username: str = Depends(verify_credentials)):
    """
    查询API额度信息（支持OpenAI官方API和千问/阿里云DashScope）
    """
    env_config = dotenv_values(".env")
    ...
    try:
        ...
        if "dashscope.aliyuncs.com" in base_url:
            ...
            return {
                ...
                "is_official": False,
                "provider": "qwen",
                "provider_name": "千问/阿里云DashScope",
                "message": "千问额度查询需要登录阿里云控制台",
                "console_url": "https://dashscope.console.aliyun.com/",
                "usage_url": "https://dashscope.console.aliyun.com/bill",
                "total_available": None,
                "total_used": None,
                "remaining": None
            }
        ...
        # 判断是否是OpenAI官方API
        is_official_api = "api.openai.com" in base_url
        ...
        if not is_official_api:
            ...
            return {
                ...
                "is_official": False,
                "provider": "custom",
                "provider_name": "第三方API",
                "message": "非官方API，无法查询额度信息",
                "total_available": None,
                "total_used": None,
                "remaining": None
            }
        ...
        async with httpx.AsyncClient() as client:
            ...
            if subscription_response.status_code != 200:
                return {
                    "error": f"查询订阅信息失败: {subscription_response.status_code}",
                    "provider": "openai",
                    "total_available": None,
                    "total_used": None,
                    "remaining": None
            ...
            # 获取总额度和过期时间
            total_available = subscription_data.get("hard_limit_usd")
            ...
            end_date = (now + timedelta(days=1)).replace(hour=0, minute=0, second=0, microsecond=0).isoformat() + "Z"
            ...
            if usage_response.status_code == 200:
                ...
                total_used = usage_data.get("total_usage", 0) / 100  # 转换为美元
            ...
            return {
                ...
                "is_official": True,
                "provider": "openai",
                "provider_name": "OpenAI官方API",
                "total_available": round(total_available, 4) if total_available else None,
                "total_used": round(total_used, 4),
                "remaining": round(remaining, 4) if remaining is not None else None,
                "access_until": datetime.fromtimestamp(access_until).strftime("%Y-%m-%d") if access_until else None
            }
...
async def get_browser_mode(username: str = Depends(verify_credentials)):
    """
    获取浏览器运行模式
    """
    env_config = dotenv_values(".env")
    run_headless = env_config.get("RUN_HEADLESS", "true").lower() != "false"

    return {
        "headless": run_headless,
        "mode": "headless" if run_headless else "headed"
    }


@app.post("/api/browser/mode")
async def set_browser_mode(request: Request, username: str = Depends(verify_credentials)):
    """
    设置浏览器运行模式（保留注释和格式）
    """
    try:
        ...
        # 读取原始.env文件内容（保留注释和格式）
        env_file = ".env"
        with open(env_file, "r", encoding="utf-8") as f:
            lines = f.readlines()

        # 更新RUN_HEADLESS的值，保留注释
        updated_lines = []
        mode_updated = False
        new_value = "false" if not headless else "true"

        for line in lines:
            # 检查是否是RUN_HEADLESS行（支持多种格式）
            if line.strip().startswith("RUN_HEADLESS"):
                # 替换值
                if "=" in line:
                    # 保留原有的引号格式
                    if '"' in line.split("=")[1]:
                        updated_lines.append(f'RUN_HEADLESS="{new_value}"\n')
        ...
        # 如果没找到RUN_HEADLESS行，在文件末尾添加
        if not mode_updated:
            updated_lines.append(f'\n# 浏览器运行模式（true=无头模式，false=有头模式）\nRUN_HEADLESS={new_value}\n')

        # 写回.env文件
        with open(env_file, "w", encoding="utf-8") as f:
            f.writelines(updated_lines)

        return {"success": True, "message": "浏览器模式已更新", "headless": headless}
...
# ============================================================================
# [已移除 - 2025-12-29] 代理池设置 API
# 原功能：获取和更新代理池基本设置（USE_PROXY_POOL, PROXY_POOL_FILE等）
# 移除原因：代理池功能不好用，后续重新规划
#
# 原端点：
#   - GET /api/settings/proxy - 获取代理池设置
#   - PUT /api/settings/proxy - 更新代理池设置
# ============================================================================

# ============================================================================
# [已移除 - 2025-12-29] 代理列表管理 API
# 原功能：获取、更新和验证代理列表
# 移除原因：代理池功能不好用，后续重新规划
#
# 原端点：
#   - GET /api/settings/proxies - 获取代理列表
#   - PUT /api/settings/proxies - 更新代理列表
#   - POST /api/settings/proxies/validate - 验证单个代理
# ============================================================================


# ============================================================================
# [已移除 - 2025-12-29] 商业代理配置 API
# 原功能：管理商业代理服务配置（快代理等）
# 移除原因：代理池功能不好用，后续重新规划
#
# 原端点：
#   - GET /api/settings/commercial-proxy - 获取商业代理配置
#   - PUT /api/settings/commercial-proxy - 更新商业代理配置
#   - POST /api/settings/commercial-proxy/extract - 从商业API提取代理
#
# 原配置文件：commercial_proxy_config.json
# ============================================================================
...
# ============================================================================
# [已移除 - 2025-12-29] 代理池管理 API
# 原功能：完整的代理池管理系统
# 移除原因：代理池功能不好用，后续重新规划
#
# 原包含内容：
#   - calculate_progress() - 计算代理池更新进度
#   - auto_update_proxies() - 智能代理自动补充系统
#   - add_proxy_pool_activity() - 代理池活动日志记录
#
# 原API端点：
#   - GET /api/proxy-pool/config - 获取代理池配置
#   - PUT /api/proxy-pool/config - 更新代理池配置
#   - GET /api/proxy-pool/status - 获取代理池状态统计
#   - POST /api/proxy-pool/extract - 立即提取代理
#   - POST /api/proxy-pool/test - 测试代理连接
#   - GET /api/proxy-pool/activity - 获取活动日志
#
# 原配置文件：
#   - proxy_pool_config.json - 代理池配置
#   - proxies.json - 代理列表
#   - proxy_pool_activity.jsonl - 活动日志
# ============================================================================


@app.post("/api/admin/change-password")
async def change_admin_password(request: Request, username: str = Depends(verify_credentials)):
    """
    修改管理员密码
    """
    try:
        ...
        if len(new_password) < 6:
            raise HTTPException(status_code=400, detail="密码长度至少需要6位")
        ...
        # 使用正则表达式替换WEB_PASSWORD的值，保留注释
        updated_lines = []
        password_updated = False

        for line in lines:
            # 检查是否是WEB_PASSWORD行（支持多种格式）
            if line.strip().startswith("WEB_PASSWORD"):
                # 替换密码值
                if "=" in line:
                    # 保留原有的引号格式
                    if '"' in line.split("=")[1]:
                        updated_lines.append(f'WEB_PASSWORD="{new_password}"\n')
                    elif "'" in line.split("=")[1]:
                        updated_lines.append(f"WEB_PASSWORD='{new_password}'\n")
                    else:
                        updated_lines.append(f'WEB_PASSWORD={new_password}\n')
