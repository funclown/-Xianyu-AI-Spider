...
# ========================================================================
# [已移除 - 2025-12-29] 动态代理管理器
# 原功能：
#   - AsyncProxyManager类 - 异步代理管理器，监控并自动更换失效代理
#   - 自动检测代理有效期并在即将失效时提取新代理
#   - 后台监控循环每2分钟检查一次代理状态
#
# 原常量：
#   - PROXY_POOL_CONFIG_FILE = "proxy_pool_config.json"
#   - PROXIES_FILE = "proxies.json"
#   - PROXY_LIFETIME_MINUTES = 30
#   - proxy_manager = None (全局实例)
#
...
""",
    ...
    # ========================================================================
    # [已移除 - 2025-12-29] 自动代理管理逻辑
    # 原功能：
    #   - 检查代理池配置文件
    #   - 检查代理文件是否存在和有效
    #   - 自动调用 update_proxies.py 提取代理
    # 移除原因：代理池功能不好用，后续重新规划
    # ========================================================================
    ...
    # ========================================================================
    # [已移除 - 2025-12-29] 动态代理监控启动/停止逻辑
    # 原功能：
    #   - 启动 AsyncProxyManager 后台监控任务
    #   - 在IP即将失效时自动更换
    #   - 任务结束时停止监控
    # 移除原因：代理池功能不好用，后续重新规划
    # ========================================================================
    ...
    for i, result in enumerate(results):
        ...
        if isinstance(result, Exception):
            print(f"任务 '{task_conf['task_name']}' 因异常而终止: {result}")
        else:
            print(f"任务 '{task_conf['task_name']}' 正常结束，本次运行共处理了 {result} 个新商品。")
